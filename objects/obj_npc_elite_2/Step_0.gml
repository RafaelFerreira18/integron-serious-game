...
if (bob_timer mod 60 == 0) {
    show_debug_message("[npc_elite_2] dist=" + string(_dist_atual)
...
if (spotted) {
    ...
    if (spotted_timer == 1) {
        show_debug_message("[npc_elite_2] SPOTTED! Iniciando contagem para batalha...");
    }

    // Apos ~70 frames (~1,15 s): dispara a batalha (executa so uma vez)
    if (spotted_timer == 70) {
        show_debug_message("[npc_elite_2] 70 frames! Tentando criar obj_elite_intro...");
...
if (!instance_exists(obj_elite_intro)) {
    if (_dist_atual <= interact_dist) {
        show_debug_message("[npc_elite_2] Jogador entrou no raio! dist=" + string(_dist_atual) + " <= " + string(interact_dist));
        spotted       = true;
        spotted_timer = 0;
    }
}
